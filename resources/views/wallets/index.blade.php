...
@section('page-subtitle', 'Kelola semua dompet digital dan rekening Anda')

@section('header-actions')
<a href="{{ route('wallets.create') }}" class="btn-secondary text-sm">
...
<div class="space-y-6 animate-fade-in">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="glass-card p-5 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-white font-bold text-lg">Rp {{ number_format($wallets->where('include_in_total',true)->sum('balance'),0,',','.') }}</p>
            </div>
        </div>
        <div class="glass-card p-5 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-white font-bold text-lg">{{ $wallets->where('type','bank')->count() }} wallet</p>
            </div>
        </div>
        <div class="glass-card p-5 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-white font-bold text-lg">{{ $wallets->where('type','e_wallet')->count() }} wallet</p>
            </div>
        </div>
    </div>
    ...
    <div class="glass-card p-16 text-center">
        ...
        <a href="{{ route('wallets.create') }}" class="btn-primary">+ Tambah Wallet Pertama</a>
    </div>
    @else
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <div class="glass-card-hover p-5 group" x-data="{ showMenu: false }">
            <div class="flex items-start justify-between mb-4">
                <div class="flex items-center gap-3">
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center text-xl font-bold shadow-lg"
                         style="background: {{ $wallet->color }}22; color: {{ $wallet->color }}; border: 1px solid {{ $wallet->color }}44">
                        {{ mb_substr($wallet->name, 0, 1) }}
                    </div>
                    <div>
                        <h3 class="text-white font-semibold">{{ $wallet->name }}</h3>
                        <span class="text-xs text-dark-400">{{ ucfirst(str_replace('_',' ',$wallet->type)) }}</span>
                    </div>
                </div>
                <div class="relative">
                    ...
                    <div x-show="showMenu" @click.away="showMenu=false" x-transition class="absolute right-0 top-8 w-40 glass-card py-1 z-10 shadow-2xl">
                        <a href="{{ route('wallets.show', $wallet) }}" class="flex items-center gap-2 px-4 py-2.5 text-sm text-dark-200 hover:text-white hover:bg-dark-700/50">
                        ...
                        <a href="{{ route('wallets.edit', $wallet) }}" class="flex items-center gap-2 px-4 py-2.5 text-sm text-dark-200 hover:text-white hover:bg-dark-700/50">
                            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125"/></svg>
                            Edit
                        </a>
                        <form action="{{ route('wallets.destroy', $wallet) }}" method="POST">
                            @csrf @method('DELETE')
                            <button type="submit" data-confirm="Yakin hapus wallet {{ $wallet->name }}?" class="flex items-center gap-2 px-4 py-2.5 text-sm text-red-400 hover:text-red-300 hover:bg-red-500/10 w-full">
            ...
            <div class="mt-3">
                ...
                <p class="text-2xl font-bold text-white">Rp {{ number_format($wallet->balance,0,',','.') }}</p>
                @if($wallet->account_number)
                <p class="text-dark-500 text-xs mt-1">{{ $wallet->account_number }}</p>
                @endif
            </div>
            ...
            <div class="mt-4 pt-4 border-t border-dark-700/30 flex items-center justify-between">
                ...
                <span class="text-dark-500 text-xs">{{ $wallet->transactions_count }} transaksi</span>
            </div>
        </div>
        @endforeach
    </div>
