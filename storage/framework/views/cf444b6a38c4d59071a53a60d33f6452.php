<?php $__env->startSection('title', 'Telegram Bot'); ?>
<?php $__env->startSection('page-title', 'Telegram Bot'); ?>
...
<div class="space-y-5 animate-fade-in">
    ...
    <div class="glass-card p-5 border border-green-500/20 bg-green-500/5">
        ...
        <div class="flex flex-wrap gap-2 items-center">
            <code class="text-sm font-mono bg-dark-800 px-3 py-2 rounded-lg text-green-300 flex-1 min-w-0 truncate"><?php echo e(route('webhook.telegram')); ?></code>
            <button onclick="navigator.clipboard.writeText('<?php echo e(route('webhook.telegram')); ?>').then(()=>showToast('URL disalin!','success'))" class="btn-secondary text-xs py-2 px-3 flex-shrink-0">Copy</button>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="glass-card p-5 text-center">
            ...
            <p class="text-3xl font-bold text-white"><?php echo e(number_format($totalMsgs)); ?></p>
        </div>
        <div class="glass-card p-5 text-center">
            <p class="text-dark-400 text-xs mb-1">Pengguna Aktif</p>
            <p class="text-3xl font-bold text-white"><?php echo e($uniqueUsers); ?></p>
        </div>
        <div class="glass-card p-5 text-center">
            ...
            <p class="text-dark-500 text-xs mt-1 truncate"><?php echo e($webhookInfo['url']); ?></p>
            <?php else: ?>
...
TELEGRAM_BOT_USERNAME=YourBotName</pre>
    ...
    <?php if(!empty($webhookInfo)): ?>
    <div class="glass-card p-5">
        <h3 class="text-white font-semibold mb-3">📡 Webhook Info</h3>
        <dl class="grid grid-cols-2 gap-3 text-sm">
            <div><dt class="text-dark-400 text-xs">URL</dt><dd class="text-white text-xs font-mono truncate"><?php echo e($webhookInfo['url'] ?? '—'); ?></dd></div>
            <div><dt class="text-dark-400 text-xs">Pending Updates</dt><dd class="text-white"><?php echo e($webhookInfo['pending_update_count'] ?? 0); ?></dd></div>
            <div><dt class="text-dark-400 text-xs">Last Error</dt><dd class="text-<?php echo e(empty($webhookInfo['last_error_message']) ? 'green' : 'red'); ?>-400 text-xs"><?php echo e($webhookInfo['last_error_message'] ?? 'None'); ?></dd></div>
            <div><dt class="text-dark-400 text-xs">Max Connections</dt><dd class="text-white"><?php echo e($webhookInfo['max_connections'] ?? 40); ?></dd></div>
        </dl>
    </div>
    <?php endif; ?>
    ...
    <div class="glass-card overflow-hidden">
        <div class="px-5 py-4 border-b border-dark-700/30 flex items-center justify-between">
            ...
            <a href="<?php echo e(route('admin.tg-logs')); ?>" class="text-primary-400 text-xs hover:text-primary-300">Lihat semua →</a>
        </div>
        <table class="data-table">
            <thead><tr><th>Waktu</th><th>User</th><th>Chat ID</th><th>Tipe</th><th>Konten</th><th>Status</th></tr></thead>
            <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $recentMsgs->take(10); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $msg): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr>
                    <td class="text-xs text-dark-400 whitespace-nowrap"><?php echo e($msg->created_at->format('d M H:i')); ?></td>
                    ...
                    <td class="max-w-xs"><p class="text-xs text-dark-300 truncate"><?php echo e(\Str::limit($msg->content, 50) ?? '(media)'); ?></p></td>
                    <td><span class="badge <?php echo e(in_array($msg->status,['processed','sent'])?'badge-success':($msg->status==='failed'?'badge-danger':'badge-pending')); ?>"><?php echo e($msg->status); ?></span></td>
                </tr>
...
async function testBot() {
    try {
        const res = await axios.post('<?php echo e(route('admin.telegram.test-connection')); ?>', {}, { headers });
...
async function setWebhook() {
    try {
        const res = await axios.post('<?php echo e(route('admin.telegram.set-webhook')); ?>', {}, { headers });
        showResult(res.data.message, res.data.success);
...
async function deleteWebhook() {
    ...
    try {
        const res = await axios.post('<?php echo e(route('admin.telegram.delete-webhook')); ?>', {}, { headers });
        showResult(res.data.message, res.data.success);
...
async function sendTest() {
    ...
    try {
        const res = await axios.post('<?php echo e(route('admin.telegram.send-test')); ?>', { chat_id: chatId, message }, { headers });
        showToast(res.data.message, res.data.success ? 'success' : 'error');
    } catch(e) { showToast('Error: ' + e.message, 'error'); }
}
</script>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /www/wwwroot/finance.arielsa.site/resources/views/admin/telegram/index.blade.php ENDPATH**/ ?>